...
architecture implementation of videomemlab_master_v1_0_M00_AXI is
  -- Component for char lookup table
  ...
  signal ascii : std_logic_vector(7 downto 0);
   ...
   type state is ( INITMEM,   -- Clear the memory
            ...
            WAIT4ASCII,	-- Read ascii from PS/2
   ...
   type COLOR_MODE is (NORMAL_MODE, FUNKY_MODE, RAINBOW_MODE, TOGGLE_COLOR);
   SIGNAL s_txt_color_mode : COLOR_MODE := NORMAL_MODE;
  ...
  -- Define constants for color palette
  constant C_COL_BLACK  : std_logic_vector(BITS_PER_PIXEL-1 downto 0) := x"0";
  constant C_COL_BLUE   : std_logic_vector(BITS_PER_PIXEL-1 downto 0) := x"1";
  constant C_COL_GREEN  : std_logic_vector(BITS_PER_PIXEL-1 downto 0) := x"2";
  constant C_COL_CYAN   : std_logic_vector(BITS_PER_PIXEL-1 downto 0) := x"3";
  constant C_COL_BROWN  : std_logic_vector(BITS_PER_PIXEL-1 downto 0) := x"4";
  constant C_COL_PURPLE : std_logic_vector(BITS_PER_PIXEL-1 downto 0) := x"5";
  constant C_COL_LIME   : std_logic_vector(BITS_PER_PIXEL-1 downto 0) := x"6";
  constant C_COL_SKY    : std_logic_vector(BITS_PER_PIXEL-1 downto 0) := x"7";
  constant C_COL_MAROON : std_logic_vector(BITS_PER_PIXEL-1 downto 0) := x"8";
  constant C_COL_VIOLET : std_logic_vector(BITS_PER_PIXEL-1 downto 0) := x"9";
  constant C_COL_IVY    : std_logic_vector(BITS_PER_PIXEL-1 downto 0) := x"A";
  constant C_COL_ARTIC  : std_logic_vector(BITS_PER_PIXEL-1 downto 0) := x"B";
  constant C_COL_RED    : std_logic_vector(BITS_PER_PIXEL-1 downto 0) := x"C";
  constant C_COL_PINK   : std_logic_vector(BITS_PER_PIXEL-1 downto 0) := x"D";
  constant C_COL_YELLOW : std_logic_vector(BITS_PER_PIXEL-1 downto 0) := x"E";
  constant C_COL_WHITE  : std_logic_vector(BITS_PER_PIXEL-1 downto 0) := x"F";

  signal txtcolor, bgcolor : std_logic_vector(BITS_PER_PIXEL-1 downto 0);
  signal s_txt_color_toggle : std_logic_vector(BITS_PER_PIXEL-1 downto 0) := C_COL_BLUE;
...
begin
  ...
  axi_wdata <= color_pixels;
    ...
    begin
        ...
        -- This state is responsible for doing write transactions to
                ...
                if (M_AXI_BVALID = '1' and axi_bready='1') then
                    if ( scan_line = CHAR_HEIGHT-1 ) then
                        scan_line <= 0;
                        if ( current_char = CHARS_PER_LINE-3 ) then -- could try - 3 here
                            current_char <= 0;
                            -- current_line <= current_line + 1; This isn't the right spot to do this (Chandler)
            ...
            when WAIT4IRQ =>
              -- This state is just a pause between READ/WRITE transactions
                if ( IRQ_I = '1' ) then  -- CDL=> May need to clear flag?
                  mst_exec_state  <= WAIT4ASCII;
                end if;
            ...
            when WAIT4ASCII =>
              ...
              if (M_AXI_RVALID = '1' and axi_rready = '1') then
                  ascii <= M_AXI_RDATA(7 downto 0); -- CDL=> Add later
      ...
      when GETPIXELS1 =>
                ...
                mst_exec_state <= GETPIXELS2;  -- CDL=> needed with ascii logic outside of master?

            when GETPIXELS2 =>
                if    ( ascii = X"10" ) then  -- FN1
                  s_txt_color_mode <= NORMAL_MODE;
                  mst_exec_state   <= WAIT4IRQ;
                elsif ( ascii = X"11" ) then  -- FN2 
                  s_txt_color_mode <= FUNKY_MODE;
                  mst_exec_state   <= WAIT4IRQ;
                elsif ( ascii = X"12" ) then  -- FN3 
                  s_txt_color_mode <= RAINBOW_MODE;
                  mst_exec_state   <= WAIT4IRQ;
                elsif ( ascii = X"13" ) then  -- FN4 
                  s_txt_color_mode <= TOGGLE_COLOR;
                  s_txt_color_toggle <= std_logic_vector(unsigned(s_txt_color_toggle) + 1);
                  mst_exec_state   <= WAIT4IRQ;
                -- elsif ( ascii = X"14" ) then  -- FN5 

                -- if the character is a carriage return, go to the next line
                pixelnum <= 0;                
                elsif ( ascii = X"0D" ) then
                    current_char <= 0;
                    if ( current_line = LINES_PER_PAGE-1 ) then
                        current_line <= 0;
                    else
                ...
                if (M_AXI_BVALID = '1' and axi_bready='1') then
                    ...
                    if ( scan_line = CHAR_HEIGHT-1 ) then
                        ...
                        if ( ascii /= X"08" ) then -- increment the cursor if it wasnt a back space
                            if ( current_char = CHARS_PER_LINE-3 ) then  -- change back to -1 here
                                current_char <= 0;
                                if ( current_line = LINES_PER_PAGE-1 ) then
                                    current_line <= 0;
                                else
  ...
  CGRAM_LUT_INST: char8x12_lookup_table
  port map
  (
    clk    => M_AXI_ACLK,
    reset  => rst_p,
...
TEXT_COLOR_GEN : process(M_AXI_ACLK)
begin
  if (rising_edge (M_AXI_ACLK)) then
    case s_txt_color_mode is
      when NORMAL_MODE =>
        txtcolor <= C_COL_WHITE;
      ...
      when FUNKY_MODE =>
        case scan_line is
          when 0 =>
            txtcolor <= C_COL_RED;
          when 1 =>
            txtcolor <= C_COL_MAROON;
          when 2 =>
            txtcolor <= C_COL_YELLOW;
          when 3 =>
            txtcolor <= C_COL_GREEN;
          when 4 =>
            txtcolor <= C_COL_LIME;
          when 5 =>
            txtcolor <= C_COL_IVY;
          ...
          when 8 =>
            txtcolor <= C_COL_SKY;
          when 9 =>
            txtcolor <= C_COL_ARTIC;
          when 10 =>
            txtcolor <= C_COL_PURPLE;
          when 11 =>
            txtcolor <= C_COL_VIOLET;
          when 12 =>
            txtcolor <= C_COL_PINK;
          when others=>
            txtcolor <= C_COL_BLUE;
      ...
      when RAINBOW_MODE =>
        txtcolor <= std_logic_vector(to_unsigned(current_char, 4));

      when TOGGLE_COLOR =>
        txtcolor <= s_txt_color_toggle;
    end case;    
  end if;
end process TEXT_COLOR_GEN;

  -- Set background and text color -- CDL=> Maybe add pattern/fade/selectablity later?
--  txtcolor <= C_COL_WHITE;
  bgcolor  <= C_COL_BLACK;

  -- Set the color_pixels word based on the reg_pixels byte. When reg_pixels(n) is '1', we use the
  -- text color for the nth nibble in color_pixels, otherwise we use the background color
  -- Colors are 4-bit : "rrgb"
  -- color_pixels     : 32 bits
  -- reg_pixels       : 8 bits
  color_map: for i in 0 to (CHAR_WIDTH-1) generate
    color_pixels(((BITS_PER_PIXEL-1)+BITS_PER_PIXEL*i) downto (i*BITS_PER_PIXEL)) <= txtcolor when (reg_pixels(i) = '1')
                                                                                else bgcolor;
  end generate color_map;
  -- CDL=> Backup if needed (simpler but longer)
